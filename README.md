...
플로팅 도구를 추가한 뒤에는 작은 인터랙션 버그가 반복해서 생겼습니다. 특히 수익 캘린더 히트맵 아이콘은 클릭했을 때 바로 열리지 않고, 현재 배치 다시 맞추기 이후 X 모양 도구 버튼을 움직이면 자산 비중 도넛이 같은 흐름으로 따라오지 않는 문제가 있었습니다.

이 문제는 각 플로팅 도구가 개별적으로 위치를 저장하고 계산하면서, 부모 도크와 자식 도크 사이의 기준 좌표가 한 프레임씩 어긋나는 구조에서 발생했습니다. 그래서 클릭과 드래그 판정을 더 엄격히 분리하고, 도넛 도크가 스파이더 차트 도크의 실제 위치를 기준으로 다시 정렬되도록 보강했습니다.

또한 도구 메뉴가 열린 상태에서는 전체 도크 스택 높이를 고려해 X 버튼의 이동 가능 범위를 제한했습니다. 화면 아래쪽에서 자식 도구만 먼저 클램프되어 도넛이 멈춘 것처럼 보이는 상황을 막기 위한 처리입니다.

이번 작업에서 어려웠던 점은 겉으로는 단순한 위치 버그처럼 보였지만, 실제로는 클릭/드래그 판정, 저장된 위치, 리셋 신호, 브라우저 하단 클램프가 동시에 얽혀 있었다는 점입니다. 최종적으로는 빌드 검증과 headless Chrome 검증으로 업로드, 잔디 히트맵 열기, 배치 재정렬, X 버튼 이동, 도넛 추적 흐름을 함께 확인했습니다.
